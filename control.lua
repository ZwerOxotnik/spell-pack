		...
		modforce = function(force,field,value)
			...
			if field == "mana" then
				...
				for _, player in pairs(force.players) do
					global.players[player.index].mana = math.max(0, math.min(global.players[player.index].max_mana + max_mana, global.players[player.index].mana + value))
				end
				return max_mana
			elseif field == "spirit" then
				local max_spirit = global.forces[force.name].max_spirit
				for _, player in pairs(force.players) do
					global.players[player.index].spirit = math.max(0, math.min(global.players[player.index].max_spirit + max_spirit, global.players[player.index].spirit + value))
				end
				return max_spirit
...
script.on_event(defines.events.on_player_cursor_stack_changed, function(event)
	...
	if global.players[event.player_index].character_build_distance_bonus_old then
		...
		global.players[event.player_index].character_build_distance_bonus_new = nil
	end
	...
	if spells[player.cursor_stack.name] then
		...
		elseif spells[spell_name].no_target then
			...
			if success then
				if global.players[player.index].bonus_effects[spell_name] then
					global.players[player.index].bonus_effects[spell_name](player)
				end
				global.players[player.index].mana = global.players[player.index].mana - spells[spell_name].mana_cost
				global.players[player.index].mana_reg = global.players[player.index].mana_reg + spells[spell_name].mana_cost/300/200
				...
				if remote.interfaces["dota_scenario_running"] then
					...
					global.players[player.index].spirit_reg = global.players[player.index].spirit_reg + spells[spell_name].spirit_cost/300/200
					global.players[player.index].max_spirit = global.players[player.index].max_spirit + spells[spell_name].spirit_cost/300
				end
				update_mana(player)
				local cd = math.max(0,(spells[spell_name].cooldown or 0) * (1-global.players[player.index].cdr))
...
function verify_force(player)
	if not global.forces[player.force.name] then
		global.forces[player.force.name] = {max_mana=0, mana_reg = 0, max_spirit = 0, spirit_reg = 0, spirit_per_kill = 0, cdr = 0, bonus_effects = {}}
	end
end
...
script.on_nth_tick(15, function(event)
	for _, player in pairs(game.players) do
		if global.players[player.index] then
			verify_force(player)
			global.players[player.index].mana = math.min(global.players[player.index].max_mana+global.forces[player.force.name].max_mana,global.players[player.index].mana+(global.players[player.index].mana_reg+global.forces[player.force.name].mana_reg)/4)
			global.players[player.index].spirit = math.min(global.players[player.index].max_spirit+global.forces[player.force.name].max_spirit,global.players[player.index].spirit+(global.players[player.index].spirit_reg+global.forces[player.force.name].spirit_reg)/4)
			update_mana(player)
		end
	end
	for unit_number, tbl in pairs(global.repairing) do
		if not tbl.entity or not tbl.entity.valid or tbl.entity.health ==0 or tbl.tick<event.tick - 31 then
			...
			if not tbl.fx or not tbl.fx.valid and tbl.entity.type ~="locomotive" and tbl.entity.type ~= "cargo-wagon" and tbl.entity.type ~= "fluid-wagon" and tbl.entity.type ~= "artillery-wagon" then
				if tbl.entity.type == "car" then
					tbl.fx = tbl.entity.surface.create_entity{name="osp_repair-sticker", position=tbl.entity.position, target=tbl.entity }
				else
	...
	for _, player in pairs(game.players) do
		...
		for spell_name, spell in pairs (spells) do
			if not spell.ignore_cooldown then
				global.players[player.index].cooldowns[spell_name] = math.max(0,(global.players[player.index].cooldowns[spell_name] or 1)-0.5)
...
script.on_event({defines.events.on_built_entity}, function(event)
	if spells[event.created_entity.name] and not spells[event.created_entity.name].trigger_created_entity then
		--if not spells[event.created_entity.name].trigger_created_entity then
			...
			if --[[global.players[player.index].mana >= spells[spell_name].mana_cost and global.players[player.index].spirit >= spells[spell_name].spirit_cost and --]](not global.verify_inventories[player.index] or not global.verify_inventories[player.index][spell_name] or global.verify_inventories[player.index][spell_name]+1 < event.tick) and (not global.players[player.index].cooldowns[spell_name] or global.players[player.index].cooldowns[spell_name] < 1)--[[player.get_main_inventory().get_item_count(spell_name) <1--]] then --and not player.cursor_stack.valid_for_read then -- mana check, cd check
			--print(2 .. event.tick)
				verify_force(player)
				...
				if success then
					...
					local cd = math.max(0,(spells[spell_name].cooldown or 0) * (1-global.players[player.index].cdr-global.forces[player.force.name].cdr))
...
script.on_event(defines.events.on_player_used_capsule, function(event)
	if event.item and spells[event.item.name] and spells[event.item.name].trigger_created_entity then
		--if not spells[event.created_entity.name].trigger_created_entity then
			...
			if --[[global.players[player.index].mana >= spells[spell_name].mana_cost and global.players[player.index].spirit >= spells[spell_name].spirit_cost and --]](not global.players[player.index].cooldowns[spell_name] or global.players[player.index].cooldowns[spell_name] < 1)--[[player.get_main_inventory().get_item_count(spell_name) <1--]] then --and not player.cursor_stack.valid_for_read then -- mana check, cd check
			--print(2 .. event.tick)
				verify_force(player)
...
script.on_event(defines.events.on_entity_died, function(event)
	if remote.interfaces["dota_scenario_running"] or not event.entity.has_flag("breaths-air") or  event.entity.type == "tree" then return end
	if settings.global["osp-show-spirits"].value then
		local characters = event.entity.surface.find_entities_filtered{name = "character", position = event.entity.position, radius = 40}
		for _, character in pairs(characters) do
			event.entity.surface.create_entity{name= "osp_spirit_projectile", position=event.entity.position, target=character, speed = 0 }
		end
	else
		local characters = event.entity.surface.find_entities_filtered{name = "character", position = event.entity.position, radius = 40}
		for _, character in pairs(characters) do
...
script.on_event(defines.events.on_trigger_created_entity, function(event)
	...
	if spells[event.entity.name] and event.source and event.source.name == "character" and spells[event.entity.name].trigger_created_entity then
	...
	if event.entity.name == "osp_absorb_explosion" then
		local character = event.entity.surface.find_entity("character", event.entity.position)
