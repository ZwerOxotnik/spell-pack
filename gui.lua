...
if gui.name == "auto_research_gui" then
	...
	local gui_width = (gui.style.minimal_width or gui.style.natural_width or 28) + (gui.style.left_padding or 0) + (gui.style.right_padding or 0) + (gui.style.left_margin or 0) + (gui.style.right_margin or 0)
	local sub_width = 0
	for a,b in pairs(gui.children) do
...
function create_gui(player)
	...
	if not global.players[player.index] then 
		global.players[player.index] = {mana=10, max_mana=100, mana_reg = 1, spirit = 0, max_spirit = 100, spirit_reg = 0, spirit_per_kill = 1, cooldowns = {}, cdr = 0, bonus_effects = {}}
	end
	verify_force(player)
	if not global.enabledbars then return end
...
function update_mana(player)
	if not player.gui.top.player_mana then return end
	local mana =  global.players[player.index].mana
	local spirit =  global.players[player.index].spirit
	local max_mana =  global.players[player.index].max_mana + global.forces[player.force.name].max_mana
	local max_spirit =  global.players[player.index].max_spirit + global.forces[player.force.name].max_spirit
...
function verify_gui_order(player)
