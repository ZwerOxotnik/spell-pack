...
function get_fireutil()
	...
	function fireutil.flamethrower_turret_extension_animation(shft, opts)
		...
		return {
			layers = fireutil.foreach(ret_layers, function(tab)
				if tab.shift then
	...
	function fireutil.flamethrower_turret_preparing_muzzle_animation(opts)
		...
		for r = 0, opts.orientation_count - 1 do
			...
			for i = 0, opts.frame_count - 1 do
				...
				local mat = matrix.compose({
					matrix.translation_vec3(math3d.vector3.mul(model.tilt_pivot, -1)), matrix.rotation_y(progress * delta_angle),
						matrix.translation_vec3(model.tilt_pivot), matrix.rotation_z(phi),
		...
		return {rotations = generated_orientations, direction_shift = fireutil.turret_gun_shift}
	end
	...
	function fireutil.flamethrower_turret_extension(opts)
		...
		return {
			north = fireutil.flamethrower_turret_extension_animation(fireutil.turret_gun_shift.north,
			...
			south = fireutil.flamethrower_turret_extension_animation(fireutil.turret_gun_shift.south,
