...
for name, spell in pairs(spells) do
	...
	elseif spell.dummy == "building" then
		...
		if (spell.spirit_cost>0) then
			item.localised_description= item.localised_description.."[color=0.8 0.8 1]"..spell.spirit_cost.." spirit[/color] "
		end
	...
	elseif spell.dummy == "grenade" then
		...
		if (spell.mana_cost>0)then
			grenade.localised_description= grenade.localised_description.."[color=0.4 0.4 1]"..spell.mana_cost.." mana[/color] "
		end
		if (spell.spirit_cost>0) then
			grenade.localised_description= grenade.localised_description.."[color=0.8 0.8 1]"..spell.spirit_cost.." spirit[/color] "
		end
		if (spell.cooldown>1) then
			grenade.localised_description= grenade.localised_description.."[color=0.99 0.99 0]"..spell.cooldown.."s CD[/color]"
		end
...
data:extend({
    ...
    icon = "__base__/graphics/icons/wooden-chest.png",
    icon_size = 32,
    flags = {"not-on-map","not-blueprintable","not-deconstructable","not-flammable","not-selectable-in-game","not-repairable","placeable-off-grid"},
    order = "s-e-w-o",
    ...
    action =
    {
      ...
      action_delivery =
      {
        ...
        target_effects =
        {
          {
            type = "create-entity",
            entity_name = "osp_absorb_explosion",
