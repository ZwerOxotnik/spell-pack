...
spells = {
		...
		func= function(player, position) 
			...
			if player.vehicle then
				...
				for i=1,60 do
					...
					end, vars = {vehicle = vehicle}})
				end
				--flames:
				for i=1, 60 do
					if not global.on_tick[game.tick + i*1] then global.on_tick[game.tick + i*1] = {} end
					table.insert(global.on_tick[game.tick + i*1], {func = function (vars)
						if vars.player.character and vars.player.character.valid then
							local pos = vars.player.position
							vars.player.surface.create_entity{name = "osp_fire_stream-"..vars.level, position = vars.player.position, force = "player", player = vars.player, source = pos, target=pos}
							local pos2 = {}
							pos2.x = pos.x-0.25+math.random()/2
							pos2.y = pos.y-0.25+math.random()/2
							vars.player.surface.create_entity{name = "osp_fire_stream-"..vars.level, position = vars.player.position, force = "player", player = vars.player, source = pos2, target=pos2}
							pos2.x = pos.x-0.25+math.random()/2
							pos2.y = pos.y-0.25+math.random()/2
							vars.player.surface.create_entity{name = "osp_fire_stream-"..vars.level, position = vars.player.position, force = "player", player = vars.player, source = pos2, target=pos2}
							pos2.x = pos.x-0.25+math.random()/2
					...
					end,vars = {player = player, level = level}})
				end
				
				global.friendly_fire[player.force.name].tick = math.max(global.friendly_fire[player.force.name].tick or 1,game.tick + 241)
				if not global.on_tick[game.tick + 241] then global.on_tick[game.tick + 241] = {} end
				table.insert(global.on_tick[game.tick + 241], {func = function (vars)
				...
				if player.character and player.character.valid then
					...
					for i, b in pairs(free_stickers) do
						if b then
							player.character.surface.create_entity{name="spellpack-speed-"..i, position = player.character.position, target = player.character}
							break
						end
					end
				end
				...
				global.friendly_fire[player.force.name].tick = math.max(global.friendly_fire[player.force.name].tick or 1,game.tick + 361)
				if not global.on_tick[game.tick + 361] then global.on_tick[game.tick + 361] = {} end
				table.insert(global.on_tick[game.tick + 361], {func = function (vars)
					if game.tick == global.friendly_fire[vars.force.name].tick then
						vars.force.friendly_fire = global.friendly_fire[vars.force.name].friendly_fire
					end
				end,vars = {force = player.force}})
	...
	["osp_recharge"] = {
		...
		func= function(player, position)
			...
			if target_entity.type ~="locomotive" and target_entity.type ~="cargo-wagon" and target_entity.type ~="fluid-wagon" and target_entity.type ~="artillery-wagon" then
				target_entity.surface.create_entity{ name="osp_electricity-sticker", position=target_entity.position, target=target_entity }
			end
			local vars = {target_entity = target_entity}
			for i=1, 10 do
				if not global.on_tick[game.tick + i*60-59] then global.on_tick[game.tick + i*60-59] = {} end
				table.insert(global.on_tick[game.tick + i*60-59], {func = function (vars)
	...
	["osp_crafting"] = {
		...
		func= function(player, position) 
			...
			if not global.on_tick[game.tick + 300] then global.on_tick[game.tick + 300] = {} end
			table.insert(global.on_tick[game.tick + 300], {func = function (vars)
				vars.player.force.manual_crafting_speed_modifier  = math.max(0,vars.old_speed + vars.player.force.manual_crafting_speed_modifier - vars.new_speed)
			end,vars = vars})
			return true
		end,
	},
	["osp_teleport"] = {
		...
		func= function(player, position) 
			...
			for _, building in pairs(buildings) do
				if building.valid and not building.has_flag("breaths-air") and (player.force.get_friend(building.force) or player.force == building.force) and building.name ~="osp_teleport" then
			...
			if closest_building then
				...
				if not global.on_tick[game.tick + 420] then global.on_tick[game.tick + 420] = {} end
				table.insert(global.on_tick[game.tick + 420], {func = function (vars)
					if vars.player.character and vars.player.character.valid then
				...
				player.surface.create_entity{name="flying-text",position=position, text="no buildings nearby", render_player_index = player.index}
				return false
			end
		end,
	},
				...
				if ent.valid and ent.health and ent.health > 0 and (ent.type =="unit" or ent.type =="character") then
					...
					if ent.stickers then
						for _, ent in pairs(ent.stickers) do
							if ent.name:sub(1,21) == "osp_fireball-sticker-" and (not sticker_lvl or tonumber(ent.name:sub(22)) > sticker_lvl) then
								sticker_lvl = tonumber(ent.name:sub(22))
							end
						end
					end
					ent.surface.create_entity{ name="osp_fireball-sticker-"..math.min(25,(sticker_lvl or level) + 1), position=ent.position, target=ent }
					if sticker_lvl then
		...
		func= function(player, position)
			...
			for _, ent in pairs(entities) do
				if ent.valid and ent.health and ent.health > 0 and (ent.type =="unit" or ent.type =="character") then
					...
					ent.surface.create_entity{ name="osp_fireball-sticker-"..math.max(0,(sticker_lvl or level) -1), position=ent.position, target=ent }
					if sticker_lvl then
	...
	["osp_timewarp"] = {
		...
		func= function(player, position) 
				...
				local running_speed_modifier = player.force.character_running_speed_modifier+player.character_running_speed_modifier+1
				player.force.character_running_speed_modifier = player.force.character_running_speed_modifier + running_speed_modifier
				...
				local vars = {player = player, running_speed_modifier = running_speed_modifier, old_game_speed = old_game_speed, attackspeeds = attackspeeds}
				if not global.on_tick[game.tick + 300] then global.on_tick[game.tick + 300] = {} end
				table.insert(global.on_tick[game.tick + 300], {func = function (vars)
					vars.player.force.character_running_speed_modifier = math.max(0,vars.player.force.character_running_speed_modifier - vars.running_speed_modifier)
	...
	["osp_repair"] = {
		...
		func= function(player, position) 
			...
			for i=1,600 do
				...
				table.insert(global.on_tick[game.tick + i], {func = function (vars)
					if vars.player.character and vars.player.character.valid then
						...
						global.players[vars.player.index].repair_radius = vars.player.surface.create_entity{name = "osp_repair_radius", position=vars.player.position}
						...
						for _, entity in pairs(surroundings) do
							if entity.valid and entity.health and entity.health >0 and entity.unit_number and not entity.has_flag("breaths-air") and not entity.has_flag("not-repairable") and entity.get_health_ratio() < 1 then
								if global.repairing[entity.unit_number] then
									...
									elseif entity.type == "car" then
										global.repairing[entity.unit_number] = {entity = entity, tick = game.tick, fx = entity.surface.create_entity{ name="osp_repair-sticker", position=entity.position, target=entity }}
									else
										global.repairing[entity.unit_number] = {entity = entity, tick = game.tick, fx = entity.surface.create_entity{name="osp_repair_fx", position = entity.position}}
									end
								end
			...
			if not global.on_tick[game.tick + 601] then global.on_tick[game.tick + 601] = {} end
			table.insert(global.on_tick[game.tick + 601], {func = function (vars)
				if global.players[vars.player.index].repair_radius and global.players[vars.player.index].repair_radius.valid then
					global.players[vars.player.index].repair_radius.destroy()
					global.players[vars.player.index].repair_radius = nil
				end
	...
	["osp_artillery"] = {
		...
		func= function(player, position)
			--if distance(player.position,position)<50 then
				...
				for i=1, 16 do
					...
					if not global.on_tick[game.tick + j] then global.on_tick[game.tick + j] = {} end
					table.insert(global.on_tick[game.tick + j], {func = function (vars)
						local pos = random_point_radius(vars.position, 7)
						local spawnpos = {pos.x-3,pos.y-30} 
						vars.surface.create_entity{name = "osp_artillery_projectile-"..vars.level, position = spawnpos, target = pos, speed = 0.9, force = vars.player.force}
					end, vars = vars})
				end
				for i=1, 19 do
					local j = 10*i
				...
				if not global.on_tick[game.tick + 110+16*5] then global.on_tick[game.tick + 110+16*5] = {} end
				table.insert(global.on_tick[game.tick + 110+16*5], {func = function (vars)
					if vars.crosshair then vars.crosshair.destroy() end
				end, vars = vars})
				return true
			--end
		end,
		entity = {
		    picture =
				{
				filename = "__spell-pack__/graphics/osp_crosshair_build.png",
				priority = "extra-high",
				width = 1050,
				height = 1050,
				--shift = util.by_pixel(-11, 4.5),
